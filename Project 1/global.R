...
game <- read.csv("./data/video_game.csv")


gl<-arrange(game, desc(Global_Sales))
newgameglobal<- gl %>% filter(., gl$Global_Sales >= 10)
plat_global2 <- ggplot(data = newgameglobal, aes(x=reorder(newgame$Platform, newgame$Global_Sales, function(x) length(x)), y=newgame$Global_Sales)) + 
...
test1 <- newgameglobal %>% group_by(Publisher) %>% summarise(volume = sum(Global_Sales)) %>% mutate(share = volume/sum(volume)*100.0) %>% arrange(desc(volume))


ggplot(test1, aes(x="", y=share, fill=Publisher)) + geom_bar(width = 1, size = 1, color= "white", stat = "identity") + coord_polar("y") +
  geom_text(aes(label = paste0(round(share), "%")),position = position_stack(vjust = 0.5)) + labs(x = NULL, y = NULL, fill = NULL, title = "Global sales") + 
  guides(fill = guide_legend(reverse = TRUE)) + theme_classic() + 
  theme(axis.line = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(),plot.title = element_text(hjust = 0.5, color = "#666666"))
